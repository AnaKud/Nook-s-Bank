...
extension TurnipViewController: UIScrollViewDelegate {
	...
	func setupBuyArea() {
		...
		buyTopView = self.customView?.setupTopView(text: AppTitle.Turnip.buyTitle, AppTitle.Turnip.buyForOval) ?? UIView()
		...
		buyPriceTextField = self.makePurchaseTextField(with: .buyPrice,
													   withPlacehoder: AppTitle.Turnip.price,
													   withImageName: AppImage.Turnip.price.rawValue)
		...
		buyCountTextField = self.makePurchaseTextField(with: .buyCount,
	...
	func setupTopTurnipView() {
		...
		turnipBackgroundView.backgroundColor = self.colors.cellColorSet.backgroundViewColor
	...
	func setupSellArea() {
		...
		sellBackgroundView.backgroundColor = self.colors.cellColorSet.backgroundViewColor
		...
		sellPriceTextField = self.makePurchaseTextField(with: .sellPrice,
														withPlacehoder: AppTitle.Turnip.sell,
														withImageName: AppImage.Turnip.sell.rawValue)
...
fileprivate extension TurnipViewController {
	func makePurchaseTextField(
		with text: TurnipTextFieldText,
		...
		textfield.textColor = self.colors.textfieldColor.textColor
		textfield.layer.cornerRadius = AppContraints.Turnip.cornerRadius
		textfield.backgroundColor = self.colors.textfieldColor.backgroundColor
		textfield.layer.borderWidth = 0.5
		textfield.layer.borderColor = self.colors.textfieldColor.borderColor
	...
	func titlePriceCountLabel(with text: String) -> UILabel {
		...
		label.textColor = self.colors.cellColorSet.itemTextColor
		return label
	}
}
